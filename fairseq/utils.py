...
def customized_softmax(x, dim, mask_z=None):
    x -= torch.max(x, dim=dim, keepdim=True)[0].detach()
    ex = torch.exp(x)
    ex = ex * mask_z + 1e-7
    res = ex / torch.sum(ex, dim=dim, keepdim=True)
    return res

'''
def get_mask_z(embed_tokens):
    # get mask z for customized softmax

    train_file = "/home/xuewyang/Xuewen/NLP/POS_EM/fairseq/examples/translation/wmt17_en_de/train.de"
    train_file_c = "/home/xuewyang/Xuewen/NLP/data-bin/wmt17_en_de/train.ca.de"

    dict_file = "/home/xuewyang/Xuewen/NLP/data-bin/wmt17_en_de/dict.de.txt"
    dict_file_c = "/home/xuewyang/Xuewen/NLP/data-bin/wmt17_en_de/dict.ca.de.txt"
    ...
    for l_d in lines_d_c:
        ...
        voc_idx_c[voc] = i + 4
        i += 1
    # mask_z = torch.empty((6632, 40)).fill_(float('-inf'))
    mask_z = np.zeros((42720, 16))
    mask_z[0, 0] = 1
    mask_z[1, 1] = 1
    mask_z[2, 2] = 1
    mask_z[3, 3] = 1
    # mask_z = torch.new_empty((6632, 40)).fill_(float('-inf'))
    ...
    for i in range(mask_z.shape[1]):
        if mask_z[:, i].sum() == 0:
            for j in range(mask_z.shape[0]):
                mask_z[j, i] = 1
...
def get_mask_z(embed_tokens):
    ...
    train_file = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.en-fr/train.fr"
    train_file_c = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.en-fr/train.ca.fr"

    dict_file = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.en-fr/dict.fr.txt"
    dict_file_c = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.en-fr/dict.ca.fr.txt"
...
def get_mask_z(embed_tokens):
    ...
    train_file = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.de-en/train.de-en.en"
    train_file_c = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.de-en/train.ca.en"

    dict_file = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.de-en/dict.en.txt"
    dict_file_c = "/home/xuewyang/Xuewen/NLP/data-bin/iwslt14.tokenized.sim.de-en/dict.ca.en.txt"
...
def get_dictionaries(dict_file):
    ...
    with open(dict_file, 'r') as f:
        ...
        for i, line in enumerate(lines):
            ...
            tok2chr[i + 4] = chr(i + 97)
    return voc2chr, chr2voc, chr2tok, tok2chr
...
def search_edit(tgt_target, distance1, distance2):
    ...
    voc2chr, chr2voc, chr2tok, tok2chr = get_dictionaries(dict_file)
    new1 = ''
    for tgt in tgt_target[:-1]:
        # pdb.set_trace()
        new1 += tok2chr[tgt]
    tokens = []
    strings = []
