    ...
    def generate(
        ...
        **kwargs
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            #     lprobs = torch.log(Pz_x) + torch.log(Py_zx)
            #
            #     return lprobs
            def compute_lprobs(Pz_x, Py_zx):
                Pz_x = Pz_x.unsqueeze(-1).repeat(1, 1, Py_zx.size(-1))
                ...
                return lprobs
            Pz_x = model.forward_decoder_c(tokens[:, :step + 1], encoder_outs)      # (240, 40)
            # cand_c = torch.argsort(Pz_x, dim=-1, descending=True)[:, :beam_size_p]
            Py_zx = model.forward_decoder(tokens[:, :step + 1], encoder_outs)          # (240, 40, 6632)
...
class EnsembleModel(torch.nn.Module):
    ...
    def __init__(self, models):
        ...
        self.incremental_states_c = None
        if all(isinstance(m.decoder, FairseqIncrementalDecoderTri) for m in models):
            self.incremental_states = {m: {} for m in models}
        if all(isinstance(m.decoder_c, FairseqIncrementalDecoder) for m in models):
            self.incremental_states_c = {m: {} for m in models}
    ...
    def _decode_one(self, tokens, model, encoder_out):
        ...
        # else:
        decoder_out = model.decoder(tokens, encoder_out)    # (240, 1, 40, 6632)
        decoder_out = decoder_out[:, -1:, :]
        # probs = torch.log(decoder_out)
        # probs = probs[:, -1, -1, :]
        probs = decoder_out[:, -1, :, :]
        return probs

    @torch.no_grad()
    def forward_decoder_c(self, tokens, encoder_outs):
        return self._decode_one_c(tokens, self.models[0], encoder_outs[0])

    def _decode_one_c(self, tokens, model, encoder_out):
        # do I need log prob or just prob ?????????????????????????????????????????????
        # if self.incremental_states_c is not None:
        #     decoder_out = model.decoder_c(tokens, encoder_out, incremental_state=self.incremental_states[model])
        # else:
        decoder_out = model.decoder_c(tokens, encoder_out)      # (240, 1, 40)
        decoder_out = decoder_out[:, -1:, :]
        # probs = torch.log(decoder_out)
        probs = decoder_out[:, -1, :]
        return probs
    ...
    def reorder_incremental_state_c(self, new_order):
        if self.incremental_states_c is None:
            return
        for model in self.models:
            model.decoder_c.reorder_incremental_state(self.incremental_states_c[model], new_order)
...
#
#     @torch.no_grad()
#     def forward_decoder(self, tokens, encoder_outs, tokens_z):
#         return self._decode_one(tokens, self.models[0], encoder_outs[0], tokens_z)
#
#     def _decode_one(self, tokens, model, encoder_out, tokens_z):
#         # if self.incremental_states is not None:
#         #     decoder_out = model.decoder(tokens, encoder_out, tokens_z, incremental_state=self.incremental_states[model])
#         # else:
#         decoder_out = model.decoder(tokens, encoder_out, tokens_z)
#         decoder_out = decoder_out[:, -1:, :]
#         probs = torch.log(decoder_out)
#         # probs = probs[:, -1, -1, :]
#         probs = probs[:, -1, -1, :]
...
#         #     decoder_out = model.decoder_c(tokens, encoder_out, incremental_state=self.incremental_states[model])
#         # else:
#         decoder_out = model.decoder_c(tokens, encoder_out)
#         # pdb.set_trace()
#         decoder_out = decoder_out[:, -1:, :]
#         probs = torch.log(decoder_out)
#         probs = probs[:, -1, :]
