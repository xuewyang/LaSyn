...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        #####
        tgt_tokens = tgt_tokens.repeat(beam_size, 1)
        #####
        ...
        reorder_state = None
        batch_idxs = None
        max_len = min(max_len, tgt_tokens.size(1) - 1)
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            #     lprobs = gamma * (torch.log(Py_zx) + torch.log(Pz_x))
            def compute_lprobs(Pz_x, Py_zx):
                # pdb.set_trace()
            ...
            Py_zx, Pz_x = model.forward_decoder(tokens[:, :step + 1], encoder_outs)
            Py_zx = Py_zx.gather(dim=1, index=tgt_tokens[:, step:step + 1].unsqueeze(2).repeat(1, 1, Py_zx.size(-1)))
            # pdb.set_trace()# (240, 40, 6632)
            # Pz_x = Pz_x.gather(dim=1,index=tgt_tokens[:, step + 1])
            ...
            if step < max_len:
                ...
                cand_scores, cand_indices, cand_beams, cand_pos = self.search.step(   # topk scores, indices and beams
                    step,
                    lprobs.view(bsz, -1, 1, self.vocab_size),
            ...
            if len(finalized_sents) > 0:
                ...
                tgt_tokens = tgt_tokens.view(bsz, -1)[batch_idxs].view(new_bsz * beam_size, -1)
